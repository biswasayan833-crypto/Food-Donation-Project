...
export async function POST(req: NextRequest) {
  try {
    // 1. Abuse Protection: Rate limit signup by client IP (5 attempts per minute)
    const clientIp = getClientIp(req);
    const rateLimit = await checkRateLimit(`auth:signup:${clientIp}`, 5, 60);
    ...
    const user = await prisma.user.create({
      data: {
        ...
        role: cleanRole,
        location: sanitizeString(location || 'New York, NY', 150),
    ...
    return NextResponse.json(
      { error: getSafeErrorMessage(error, 'Failed to create user account.') },
      { status: 500 }
    );
  }
}
